...
/**
 * Lightweight tracer that creates child [TraceContext] spans in the coroutine context. Tracks
 * active spans in a [ConcurrentHashMap] for diagnostics.
 */
public object DqxnTracer {
  ...
  /**
   * Runs [block] in a child span under the current trace (or a new trace if none exists). The span
   * is automatically tracked and removed on completion.
   */
  public suspend fun <T> withSpan(
    ...
    val childTrace = parentTrace?.childSpan() ?: TraceContext.newTrace()
