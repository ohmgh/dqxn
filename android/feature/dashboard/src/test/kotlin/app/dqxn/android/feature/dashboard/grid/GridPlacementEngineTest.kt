...
class GridPlacementEngineTest {
  ...
  fun `findOptimalPosition respects no-straddle snap on configuration boundary`() {
    ...
    val fullyInside =
      position.col >= 0 && widgetRight <= 10 && position.row >= 0 && widgetBottom <= 12
    val fullyOutside =
      position.col >= 10 || widgetRight <= 0 || position.row >= 12 || widgetBottom <= 0
