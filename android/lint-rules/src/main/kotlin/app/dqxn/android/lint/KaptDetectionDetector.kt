 ...
 *
 * KAPT breaks Gradle configuration cache and is slower than KSP.
 * All annotation processing in DQXN must use KSP exclusively.
 */
class KaptDetectionDetector : Detector(), GradleScanner {
    ...
    override fun checkMethodCall(
        ...
        if (statementTrimmed == "id" || statementTrimmed == "apply") {
            val pluginId = unnamedArguments.firstOrNull()?.removeSurrounding("'")?.removeSurrounding("\"")
