...
class DateStackRendererTest : WidgetRendererContractTest() {
  ...
  fun `accessibility contains date`() {
    val d = DateStackRenderer().accessibilityDescription(createTestWidgetData())
  ...
  fun `accessibility no data`() {
    assertThat(DateStackRenderer().accessibilityDescription(WidgetData.Empty)).contains("no data")
  }
}
