...
class DateGridRendererTest : WidgetRendererContractTest() {
  ...
  @Test fun `accessibility contains date`() { val d = DateGridRenderer().accessibilityDescription(createTestWidgetData()); assertThat(d).contains("January"); assertThat(d).contains("15"); assertThat(d).contains("2026") }
  @Test fun `accessibility no data`() { assertThat(DateGridRenderer().accessibilityDescription(WidgetData.Empty)).contains("no data") }
}
