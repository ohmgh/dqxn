...
class BatteryProviderTest : DataProviderContractTest() {
  ...
  @Test
  fun `battery level 75 of 100`() {
    assertThat(
        BatteryProvider.extractBatterySnapshot(createBatteryIntent(level = 75, scale = 100))!!.level
      )
      .isEqualTo(75)
  }

  @Test
  fun `battery level 50 of 200`() {
    assertThat(
        BatteryProvider.extractBatterySnapshot(createBatteryIntent(level = 50, scale = 200))!!.level
      )
      .isEqualTo(25)
  }

  @Test
  fun `charging - charging status`() {
    assertThat(
        BatteryProvider.extractBatterySnapshot(
            createBatteryIntent(status = BatteryManager.BATTERY_STATUS_CHARGING)
          )!!
          .isCharging
      )
      .isTrue()
  ...
  @Test
  fun `charging - full status`() {
    assertThat(
        BatteryProvider.extractBatterySnapshot(
            createBatteryIntent(status = BatteryManager.BATTERY_STATUS_FULL)
          )!!
          .isCharging
      )
      .isTrue()
  ...
  @Test
  fun `charging - discharging status`() {
    assertThat(
        BatteryProvider.extractBatterySnapshot(
            createBatteryIntent(status = BatteryManager.BATTERY_STATUS_DISCHARGING)
          )!!
          .isCharging
      )
      .isFalse()
  ...
  @Test
  fun `temperature 285 tenths to 28_5 celsius`() {
    assertThat(
        BatteryProvider.extractBatterySnapshot(createBatteryIntent(temperature = 285))!!.temperature
      )
      .isEqualTo(28.5f)
  }

  @Test
  fun `temperature null when zero`() {
    assertThat(
        BatteryProvider.extractBatterySnapshot(createBatteryIntent(temperature = 0))!!.temperature
      )
      .isNull()
  }

  @Test
  fun `returns null for invalid level`() {
    assertThat(BatteryProvider.extractBatterySnapshot(createBatteryIntent(level = -1))).isNull()
  }

  @Test
  fun `returns null for zero scale`() {
    assertThat(BatteryProvider.extractBatterySnapshot(createBatteryIntent(scale = 0))).isNull()
  }
  ...
  fun `sticky broadcast delivers first emission immediately`() = runTest {
    val ctx: Context =
      mockk(relaxed = true) {
        every { registerReceiver(any<BroadcastReceiver>(), any<IntentFilter>()) } answers
          {
            firstArg<BroadcastReceiver>().onReceive(this@mockk, createBatteryIntent())
            createBatteryIntent()
          }
      }
  ...
  fun `subsequent broadcasts update the flow`() = runTest {
    ...
    val ctx: Context =
      mockk(relaxed = true) {
        every { registerReceiver(capture(receiverSlot), any<IntentFilter>()) } answers
          {
            firstArg<BroadcastReceiver>()
              .onReceive(this@mockk, createBatteryIntent(level = 50, scale = 100))
            createBatteryIntent()
          }
      }
    ...
    receiverSlot.captured.onReceive(
      ctx,
      createBatteryIntent(level = 80, scale = 100, status = BatteryManager.BATTERY_STATUS_CHARGING)
    )
    delay(50)
  ...
  companion object {
    internal fun createBatteryIntent(
      level: Int = 75,
      scale: Int = 100,
