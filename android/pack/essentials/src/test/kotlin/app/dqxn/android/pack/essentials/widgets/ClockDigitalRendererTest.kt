...
class ClockDigitalRendererTest : WidgetRendererContractTest() {
  ...
  override fun createTestWidgetData(): WidgetData {
    val zdt = ZonedDateTime.of(LocalDateTime.of(2026, 1, 15, 14, 30, 45), ZoneId.of("America/New_York"))
  ...
  @Test fun `accessibilityDescription includes formatted time`() { assertThat(ClockDigitalRenderer().accessibilityDescription(createTestWidgetData())).contains("14:30") }
  @Test fun `accessibilityDescription returns no data for empty`() { assertThat(ClockDigitalRenderer().accessibilityDescription(WidgetData.Empty)).contains("no data") }
  @Test fun `getDefaults wider with showSeconds`() {
  ...
  @Test fun `12h format shows 2_30`() {
    val desc = ClockDigitalRenderer().accessibilityDescriptionWithSettings(createTestWidgetData(), mapOf("use24HourFormat" to false, "showSeconds" to false))
