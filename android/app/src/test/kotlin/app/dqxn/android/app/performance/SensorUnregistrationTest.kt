...
/**
 * Source-code analysis test verifying correct sensor lifecycle management in callbackFlow
 * providers.
 *
...
class SensorUnregistrationTest {
  ...
  /**
   * Provider source files that use callbackFlow for sensor/callback registration. Paths relative to
   * the android/ project root.
   */
  private val providerFiles: List<String> =
    listOf(
  ...
  /**
   * Valid unregistration patterns inside awaitClose blocks. Each callbackFlow that registers a
   * listener/receiver/callback must unregister in awaitClose.
   */
  private val unregistrationPatterns: List<Regex> =
    listOf(
  ...
  fun `every callbackFlow has matching awaitClose with unregistration`() {
    for (relativePath in providerFiles) {
      ...
      assertWithMessage("Provider file must exist: $relativePath").that(file.exists()).isTrue()
      ...
      val sourceWithoutImports =
        source.lines().filter { !it.trimStart().startsWith("import ") }.joinToString("\n")
      ...
      assertWithMessage(
          "$fileName: callbackFlow count ($callbackFlowCount) must equal awaitClose count ($awaitCloseCount)"
        )
  ...
  fun `all declared provider files exist and use callbackFlow`() {
    for (relativePath in providerFiles) {
      ...
      assertWithMessage("${file.name} must use callbackFlow").that(hasCallbackFlow).isTrue()
    }
  }

  /**
   * Resolves a provider file path relative to the android/ directory. Handles both Gradle test
   * execution (working dir = android/) and IDE execution.
   */
  ...
  /**
   * Extracts the body text of each `awaitClose { ... }` block. Handles single-line `awaitClose {
   * foo() }` and multi-line blocks.
   */
  ...
  /**
   * Finds the index of the closing brace matching the opening brace at [openIndex]. Handles nested
   * braces, string literals, and single-line comments.
   */
