...
/**
 * Tracks app session lifecycle and fires [AnalyticsEvent.SessionStart] /
 * [AnalyticsEvent.SessionEnd] with quality metrics (F12.7): jank%, peak thermal level, render
...
constructor(
  ...
  public fun onSessionEnd(editCount: Int) {
    ...
    val providerErrors =
      statuses.values.count { it.status == WidgetHealthMonitor.Status.STALE_DATA }
  ...
  /**
   * Computes jank percentage from frame histogram. Jank = frames > 16ms (buckets 3, 4, 5) /
   * total * 100.
   */
