...
Kotlin-only (no Java). Jetpack Compose + Material 3. Hilt + KSP (no KAPT — required for configuration cache). Proto DataStore + Preferences DataStore. kotlinx-collections-immutable. kotlinx.serialization.
...
- Events flow through sealed `DashboardEvent` → routed to coordinator. All mutations on `Dispatchers.Main` via sequential `Channel`.

### Compose performance (60fps with 12+ widgets)
- **State read deferral**: High-frequency values (`State<Float>`) read in draw phase (`drawWithCache`/`onDrawBehind`), not composition
...
- **Typed DataSnapshot**: Sealed subtypes per data type (no `Map<String, Any>` boxing). Target <4KB allocation/frame in steady state.
- **movableContentOf**: Use for widget slots during drag reordering to avoid detach/reattach
...
class BatteryTempRenderer @Inject constructor() : WidgetRenderer {
    ...
    override val compatibleDataTypes = listOf("BATTERY")
    ...
    override fun accessibilityDescription(data: WidgetData): String =
        "Battery temperature: ${data.formatted("temperature")}"
    ...
    override fun Render(
        ...
        Box(modifier = modifier) {
            Text("Battery Temp", color = style.primaryTextColor)
        }
    }
}
```
...
- **Widget error isolation**: Each widget in a catch boundary (composition + effects). Failed widget → fallback UI, never app crash.
- **ConnectionStateMachine**: FSM with validated transitions. No ad-hoc `MutableStateFlow<ConnectionState>`.
- **Thermal adaptation**: `ThermalManager` → `RenderConfig`. Glow disabled at DEGRADED, frame rate reduced via `FramePacer` using `Choreographer`.
...
- `AnrWatchdog` on dedicated thread — captures stack + ring buffer context on 3s main thread stall
...
| Why typed DataSnapshot, not Map? | `ImmutableMap<String, Any>` boxes every `Float`/`Int`. At 60 emissions/sec across 12 widgets = 720 garbage objects/sec. |
| Why custom LazyLayout, not LazyGrid? | Grid placement is absolute-position, not flow-based. `LazyLayout` + custom `MeasurePolicy` matches the grid model. |
