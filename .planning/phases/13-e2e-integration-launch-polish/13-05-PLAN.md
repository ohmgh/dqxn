...
<task type="auto">
  <name>Task 1: MultiPackHiltTest — Hilt binding validation for all 3 packs</name>
  <files>
    android/app/src/test/kotlin/app/dqxn/android/app/integration/MultiPackHiltTest.kt
  </files>
  <action>
  ...
  <action>
    1. **ChaosCorrelationE2ETest** (JUnit4 instrumented test):
       ...
       - **Test: deterministic chaos correlation** (`@Test fun chaosCorrelationSeed42()`):
         ...
         - Wait for faults to inject: `client.awaitCondition("list-diagnostics", "data.snapshots", { it.isNotEmpty() }, timeoutMs = 10000)`. Do NOT use `Thread.sleep` — use deterministic condition polling only.
