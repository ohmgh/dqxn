...
<task type="auto">
  <name>Task 1: SensorUnregistrationTest — verify callbackFlow awaitClose in all providers</name>
  <files>
    android/app/src/test/kotlin/app/dqxn/android/app/performance/SensorUnregistrationTest.kt
  </files>
  <action>
...
<task type="auto">
  ...
  <action>
    ...
    Both tests: `@LargeTest` annotation, `@Ignore("Requires real device for reliable battery measurement")` can be added if needed for CI filtering.
    PITFALL: Emulator battery is virtual -- tests validate infrastructure compiles and runs, but meaningful drain numbers require physical device.
  </action>
  <verify>
    <automated>cd android && ./gradlew :app:compileDebugAndroidTestKotlin --console=plain 2>&1 | tail -5</automated>
  </verify>
  <done>BatterySoakTest measures 30-min screen-on drain with 12 widgets, extrapolates to hourly rate, asserts < 5%. BackgroundBatterySoakTest measures backgrounded drain and verifies sensor unregistration. Both compile; real execution requires physical device.</done>
</task>
