...
must_haves:
  truths:
    - "Compose stability script parses compiler report files and fails on unstable classes"
    - "APK size script measures release APK and fails when > 30MB"
    - "Build time script measures clean assembleDebug and fails when > 120s"
  artifacts:
    ...
    - path: "android/scripts/check-build-time.sh"
      provides: "Clean build time gate checking assembleDebug < 120s"
      contains: "120"
  key_links:
...
If no report files exist for a module, print a warning but don't fail — the module may not have been built yet or may not have Compose compiler enabled.
  </action>
  <verify>
...
2. Runs `./gradlew clean` then `time ./gradlew assembleDebug --console=plain --no-build-cache` with `TIMEFORMAT='%R'`.
3. Captures elapsed time in seconds.
4. Threshold: < 120s (NF35).
5. Prints `[build-time] Clean build: {X}s (threshold: 120s)`.
6. Exits 1 if >= 120s, exits 0 otherwise.

Note: This script actually runs a build, so it's a longer-running CI step. It should be called explicitly, not as part of every commit.
...
Use `set -euo pipefail`.
  ...
  <done>check-apk-size.sh gates APK < 30MB with cross-platform stat support. check-build-time.sh runs clean build and gates < 120s. Both are executable with valid bash syntax.</done>
</task>
...
- check-build-time.sh runs clean build, fails on >= 120s
