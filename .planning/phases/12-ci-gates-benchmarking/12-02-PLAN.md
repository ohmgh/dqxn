...
3. **Incremental build gate (NF35):** After clean build completes, invalidate one source file to trigger an incremental build. Use a safe touch-based approach: `touch app/src/main/kotlin/app/dqxn/android/DqxnApplication.kt` — this updates the mtime without modifying content, which is sufficient to trigger Gradle's up-to-date check. Then runs `time ./gradlew assembleDebug --console=plain` (without `--no-build-cache` and without `clean`). Captures elapsed time. Threshold: < 15s. Prints `[build-time] Incremental build: {X}s (threshold: 15s)`.
   **Note:** `touch` is idempotent and crash-safe — no risk of leaving source files dirty if the script is interrupted.
