...
must_haves:
  truths:
    ...
    - "CI gates orchestrator invokes all 8 gate scripts with aggregate pass/fail"
...
5. Runs gates in order (each independent, failure of one doesn't skip the rest):
   ...
   **Gate 8 gates are the total.** R8 mapping file existence was considered for Gate 9 but rejected — if `assembleRelease` succeeds with R8 enabled, the mapping file always exists. It's a tautological check, not a meaningful quality gate.

6. Gates 6, 7, and 8 are conditionally run:
   ...
   - Gate 8 only when release APK exists (requires prior `assembleRelease`)
...
Add Kover verification configuration. Kover 0.9.7 (Feb 2026) uses the `bound { minValue }` DSL — **not** the deprecated `minBound()` API:
```kotlin
kover {
    reports {
        verify {
...
- ci-gates.sh orchestrates 8 gates with aggregate summary and conditional skip for build-time, benchmark, and baseline-profile gates
