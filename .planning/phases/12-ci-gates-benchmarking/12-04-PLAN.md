...
must_haves:
  truths:
    ...
    - "CI gates orchestrator invokes all gate scripts with aggregate pass/fail"
...
4. Thresholds from architecture doc / migration doc:
   - P50 frame time: < 8.0ms
   - P95 frame time: < 12.0ms
   - P99 frame time: < 16.0ms
   - Cold startup median: < 1500.0ms
...
5. Runs gates in order (each independent, failure of one doesn't skip the rest):
   ...
   **Gate 6: Build time** — `$SCRIPT_DIR/check-build-time.sh` (skipped in fast mode, gated by `--full` flag)
...
6. Gates 6 and 7 are conditionally run:
   - Gate 6 only when `--full` is passed (it runs a clean build which is slow)
...
Note: The Kover DSL may differ between versions. If `minBound(90)` is not the correct API for Kover 0.9.7, check `kover { reports { verify { rule { bound { minValue = 90 } } } } }` as an alternative. The executor should verify the API against the actual Kover version.
  ...
  <verify>
    <automated>cd /Users/ohm/Workspace/dqxn/android && test -x scripts/ci-gates.sh && bash -n scripts/ci-gates.sh && grep -q "kover" feature/dashboard/build.gradle.kts && echo "PASS" || echo "FAIL"</automated>
  </verify>
...
<success_criteria>
- check-benchmark.sh parses benchmark JSON with P50 < 8ms, P95 < 12ms, P99 < 16ms, startup < 1500ms thresholds
- ci-gates.sh orchestrates 7 gates with aggregate summary and conditional skip for build-time and benchmark gates
