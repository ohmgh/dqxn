...
4. Task 3 will use the deferred skip for Gate 8 instead
  ...
  <done>baselineprofile plugin 1.5.0-alpha03 active on both :baselineprofile (producer) and :app (app-target). Benchmark library bumped to 1.5.0-alpha03. UiAutomator bumped to 2.4.0-beta01. Profile injection into release APK is now automated. NF9 fully satisfied.</done>
</task>
...
```
  ...
  <verify>
    <automated>cd /Users/ohm/Workspace/dqxn/android && bash -n scripts/ci-gates.sh && echo "PASS: valid syntax" || echo "FAIL"</automated>
  </verify>
  <done>Gate 8 is either: (a) active and checking for baseline profile in release APK (plugin works), or (b) deferred skip with clear message (plugin fallback). Script syntax valid.</done>
</task>
