...
1. **`steadyState12Widgets()`** — `rule.measureRepeated()` with:
   ...
   - `setupBlock = { startActivityAndWait(); device.waitForIdle() }` — to ensure 12 widgets are active before measurement, the setupBlock must populate the dashboard: issue 12 agentic `widget-add` commands (one per widget type from the essentials pack) via `device.executeShellCommand("am broadcast ...")` or call `preset-load` via the agentic ContentProvider to load a pre-seeded 12-widget layout. The app must not launch with an empty DataStore.
