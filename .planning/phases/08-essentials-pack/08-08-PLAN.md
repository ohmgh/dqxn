...
<task type="auto">
  <name>Task 3: Multi-slot integration proof, on-device wiring, and stability soak</name>
  <files>
    android/pack/essentials/src/test/kotlin/app/dqxn/android/pack/essentials/integration/MultiSlotBindingTest.kt
  </files>
  <action>
    This task covers ROADMAP success criteria 2, 3, and 4 — the three items that prove end-to-end wiring beyond unit tests.
    ...
    **Part B: On-device wiring verification (criterion 3)**
    ...
    After installing the debug APK on a connected device/emulator, run the agentic verification sequence:
    ...
    for typeId in essentials:speedometer essentials:clock essentials:clock-analog essentials:date-simple essentials:date-stack essentials:date-grid essentials:compass essentials:battery essentials:speedlimit-circle essentials:speedlimit-rect essentials:shortcuts essentials:solar essentials:ambient-light; do
      adb shell content call --uri content://app.dqxn.android.agentic --method add-widget --arg "$typeId"
      sleep 1
    done

    # Dump health for each widget — assert ACTIVE status
    adb shell content call --uri content://app.dqxn.android.agentic --method dump-health
    # Parse output: every widget entry should show status=ACTIVE

    # Query semantics — assert visible nodes with non-empty contentDescription
    adb shell content call --uri content://app.dqxn.android.agentic --method query-semantics --arg "dashboard_grid"
    # Parse output: 13 widget nodes, each with non-empty contentDescription
    ```

    Parse the agentic output to verify:
    - All 13 widgets added successfully (no "unknown typeId" errors)
    ...
    **If no device/emulator is available:** Document this as a manual verification step and create a shell script `verify-on-device.sh` that the user can run later. Mark the task done with a note.
    ...
    **Part C: 60-second stability soak (criterion 4)**
    ...
    adb shell content call --uri content://app.dqxn.android.agentic --method dump-health
    # Parse: no safe_mode=true flag in output

    # Check widget crash count — must be < 3 (safe mode threshold is >3 in 60s)
    # Look for crash_count in dump-health output
    ```

    Assert:
    - Safe mode is NOT active after 60 seconds
    - No widget shows ERRORED status
    - No provider shows DISCONNECTED status (except GPS-dependent ones on emulator without GPS simulation)
  </action>
  <verify>
