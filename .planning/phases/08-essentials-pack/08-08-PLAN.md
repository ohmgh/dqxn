...
<task type="auto">
  <name>Task 3: AddWidgetHandler, multi-slot integration proof, on-device wiring, and stability soak</name>
  <files>
    android/pack/essentials/src/test/kotlin/app/dqxn/android/pack/essentials/integration/MultiSlotBindingTest.kt
    android/app/src/debug/kotlin/app/dqxn/android/debug/handlers/AddWidgetHandler.kt
    android/app/src/test/kotlin/app/dqxn/android/debug/handlers/AddWidgetHandlerTest.kt
  ...
  <action>
    This task covers ROADMAP success criteria 2, 3, and 4 — the three items that prove end-to-end wiring beyond unit tests. It also creates the `AddWidgetHandler` agentic command required for on-device verification.

    **Part 0: Create AddWidgetHandler agentic command**
    ...
    **Important:** The `AgenticContentProvider` uses a response-file protocol — all responses are written to a temp file in cacheDir, and the returned Bundle contains only the file path. The adb commands in Parts B/C must account for this.

    **Part A: merge()+scan() multi-slot integration test (criterion 2)**

    Create `MultiSlotBindingTest.kt` (JUnit5) in the integration test package. This test proves that `WidgetDataBinder` (from Phase 7's `:feature:dashboard`) correctly uses merge()+scan() — NOT combine() — for multi-slot widgets.

    Test setup:
    - Use `DashboardTestHarness` from `:feature:dashboard` testFixtures
    - Create a `SpeedometerRenderer` instance
    - Create 3 fake `DataProvider` implementations:
      - `FakeSpeedProvider`: emits `SpeedSnapshot(speedMps = 16.67f, ...)` immediately
      - `FakeAccelerationProvider`: emits `AccelerationSnapshot(acceleration = 2.5f, ...)` after 2-second delay
      - `FakeSpeedLimitProvider`: emits `SpeedLimitSnapshot(speedLimitKph = 60f, ...)` after 3-second delay
    - Construct a `WidgetDataBinder` (or use the harness wiring if available) and bind all 3 providers to the speedometer widget instance

    Assertions:
    1. After 500ms (before acceleration emits): `widgetData.snapshot<SpeedSnapshot>()` is non-null — speed data arrives immediately
    2. After 500ms: `widgetData.snapshot<AccelerationSnapshot>()` is null — acceleration hasn't emitted yet
    3. After 500ms: SpeedometerRenderer's `accessibilityDescription(widgetData)` includes "60" (60 kph from speed) — renders with partial data
    4. After 2.5s: `widgetData.snapshot<AccelerationSnapshot>()` is non-null — acceleration arrived independently
    5. After 2.5s: `widgetData.snapshot<SpeedSnapshot>()` is still non-null — speed data was NOT cleared by acceleration arrival
    6. After 3.5s: all 3 slots populated, `accessibilityDescription` includes "Over limit" (speed > limit triggers warning)

    This test FAILS if combine() is used — combine() would block until all 3 emit, so assertion 1 would fail (no speed data at 500ms).

    Use `StandardTestDispatcher` and `advanceTimeBy()` for deterministic timing. Use Turbine for flow assertions.

    **If `WidgetDataBinder` is not directly instantiable** (requires too many Phase 7 coordinator dependencies), fall back to a simpler proof: construct `WidgetData` manually via progressive slot population:
    ```
    val data0 = WidgetData.Empty
    val data1 = data0.withSlot(SpeedSnapshot::class, speedSnapshot)
    // Assert speedometer accessibilityDescription works with data1 (speed only)
    val data2 = data1.withSlot(AccelerationSnapshot::class, accelSnapshot)
    // Assert speed still present in data2, acceleration also present
    val data3 = data2.withSlot(SpeedLimitSnapshot::class, limitSnapshot)
    // Assert all 3 present, "Over limit" in description
    ```
    This proves the WidgetData accumulation model works correctly for merge()+scan() — each slot update preserves existing slots.

    **Part B: On-device wiring verification (criterion 3)**

    After installing the debug APK on a connected device, run the agentic verification sequence. **Note:** The `AgenticContentProvider` uses a response-file protocol — responses are written to a temp file in cacheDir. The returned Bundle contains `filePath=<path>`. All adb commands must extract the file path and `cat` it to read the actual JSON response.

    Helper function for reading agentic responses. **Important:** `AgenticContentProvider.parseParams` expects a JSON object string — bare strings are not parsed. The helper passes `arg` through without extra quoting so callers must provide valid JSON when args are needed.
    ```bash
    agentic_call() {
      local method="$1"
      local arg="$2"
    ...
    ```bash
    # Install debug APK and launch
    ./gradlew installDebug --console=plain
    adb shell am start -n app.dqxn.android/.MainActivity
    sleep 5

    # Add each of the 13 widget types via agentic command
    # Args must be JSON objects — parseParams only extracts JsonObject keys
    ...
    # Dump health — assert ACTIVE status for all widgets (no arg needed)
    echo "=== HEALTH CHECK ==="
    agentic_call dump-health

    # Query semantics — assert visible nodes with non-empty contentDescription
    echo "=== SEMANTICS CHECK ==="
    agentic_call query-semantics '{"testTag":"dashboard_grid"}'
    ```

    Parse the agentic JSON output to verify:
    ...
    **Part C: 60-second stability soak (criterion 4)**

    With all 13 widgets placed (from Part B):
    ```bash
    # Wait 60 seconds — widgets rendering, providers emitting
    echo "Starting 60-second stability soak..."
    sleep 60

    # Check safe mode was NOT triggered
    echo "=== POST-SOAK HEALTH CHECK ==="
    agentic_call dump-health
    # Parse JSON: no safe_mode=true flag in output
    # Check widget crash count — must be < 3 (safe mode threshold is >3 in 60s)
    ```

    Assert:
    - Safe mode is NOT active after 60 seconds
    - No widget shows ERRORED status
    - No provider shows DISCONNECTED status (except GPS-dependent ones if no GPS signal available — acceptable)
  </action>
  <verify>
    <automated>cd /Users/ohm/Workspace/dqxn/android && ./gradlew :pack:essentials:testDebugUnitTest --tests "*.MultiSlotBindingTest" --console=plain 2>&1 | tail -15</automated>
    <manual>On-device: install debug APK, add all 13 widgets, wait 60s, verify dump-health shows ACTIVE and no safe mode</manual>
  </verify>
  <done>AddWidgetHandler created and tested. MultiSlotBindingTest proves merge()+scan() independent slot arrival — speed data available before acceleration/speed limit emit. On-device: all 13 widgets added via add-widget, dump-health shows ACTIVE, query-semantics confirms non-empty contentDescription. 60-second soak completes without safe mode trigger.</done>
</task>
...
10. On-device: add-widget for all 13 types → dump-health ACTIVE for each
11. On-device: 60-second soak → safe mode NOT triggered
</verification>

<success_criteria>
- All 7 Phase 8 gate criteria from ROADMAP pass:
  ...
  3. End-to-end wiring: on-device add-widget + dump-health ACTIVE + query-semantics non-empty contentDescription for all 13 types
  4. Stability soak: 60-second soak with all 13 widgets — safe mode not triggered
  5. Regression gate: full project tests pass with pack in dependency graph
  6. KSP generation produces valid Hilt module + manifest
