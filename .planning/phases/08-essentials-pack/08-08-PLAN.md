...
<objective>
Verify end-to-end integration: all contract tests green, KSP generation works, full project builds with the pack, and regression suite passes.

Purpose: This is the Phase 8 gate — all four criteria from the roadmap must pass before Phase 9 can start. This plan runs the full verification pipeline and creates a lightweight integration test that documents the pack's widget/provider inventory.

Output: All tests green, full build successful, integration verification test.
</objective>
...
<success_criteria>
- All 4 Phase 8 gate criteria from ROADMAP pass:
  1. Contract tests green (13 widgets + 9 providers)
  2. KSP generation produces valid Hilt module + manifest
  3. Full project tests pass with pack in dependency graph
