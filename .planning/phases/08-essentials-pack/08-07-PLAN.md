...
<task type="auto">
  ...
  <action>
    **EssentialsThemeProvider** — provides 2 free bundled themes:
    ...
    - `@Singleton class EssentialsThemeProvider @Inject constructor()` implementing the theme provider interface (check `:sdk:contracts` for `ThemeProvider` or similar)
    - Loads themes from `themes/slate.theme.json` and `themes/minimalist.theme.json` in resources
    - Uses `kotlinx.serialization` for JSON parsing
    - Both themes: `requiredAnyEntitlement = null` (free)
    - Returns `List<ThemeSpec>` with metadata (id, displayName, mode, isFree)
