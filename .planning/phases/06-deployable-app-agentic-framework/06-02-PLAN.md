...
files_modified:
  ...
  - android/app/src/test/kotlin/app/dqxn/android/BuildConfigTest.kt
autonomous: true
requirements: [F1.1, F1.13, F1.23, F13.4, F13.8, NF20, NF22, NF23]
...
<task type="auto">
  ...
  <action>
    ...
    - Add `buildFeatures { buildConfig = true }` for DEMO_MODE_AVAILABLE
    - Add `buildTypes { debug { buildConfigField("Boolean", "DEMO_MODE_AVAILABLE", "true") } release { buildConfigField("Boolean", "DEMO_MODE_AVAILABLE", "false") } }`
...
<task type="auto">
  ...
  <action>
    ...
    - JUnit5 @Tag("fast")
    - Test: `BuildConfig.DEMO_MODE_AVAILABLE` equals `true` in debug build (F13.4)
    - Test: `BuildConfig.APPLICATION_ID` equals `"app.dqxn.android"` (NF22 — verify build config wiring)
    - NOTE: Unit tests run against debug build type, so `DEMO_MODE_AVAILABLE` should be `true`. Release-side verification (`== false`) is covered by Plan 06-04 Task 2 via generated source inspection.
  </action>
  <verify>
    <automated>cd /Users/ohm/Workspace/dqxn/android && ./gradlew :app:testDebugUnitTest --console=plain 2>&1 | tail -10</automated>
    <sampling_rate>run after task commits</sampling_rate>
  </verify>
  <done>CrashRecovery tests pass (threshold detection, boundary conditions, corruption handling). AlertSoundManager stub test passes. StubEntitlementManager tests pass (free-only behavior). BuildConfigTest passes (DEMO_MODE_AVAILABLE == true in debug). All test XML in build/test-results/.</done>
</task>
