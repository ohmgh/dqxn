...
<task type="auto">
  ...
  <action>
    ...
    - Verify in PresetLoader unit test that default preset excludes GPS-dependent widgets. The test should already exist in `:data` (from Phase 5 PresetLoader). If not, add assertion: default preset widget typeIds should NOT contain "essentials:speedometer", "essentials:speed-limit-circle", "essentials:speed-limit-rect" — only "essentials:clock-digital", "essentials:battery", "essentials:date-simple" (or similar non-GPS widgets).
  ...
  <verify>
    <automated>cd /Users/ohm/Workspace/dqxn/android && ./gradlew :feature:dashboard:testDebugUnitTest --tests "*.OverlayNavHostRouteTest" --console=plain 2>&1 | tail -20</automated>
  </verify>
  <done>DashboardScreen navigates to onboarding on first run, all 7 routes verified via integration test, source-varying transitions configured</done>
</task>
