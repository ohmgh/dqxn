...
# Note: SessionEvent + EventType are now in :sdk:observability (session package), not :feature:diagnostics.
# SessionRecorder is still in :feature:diagnostics and can be injected directly by DiagnosticsViewModel.
files_modified:
...
must_haves:
  truths:
    ...
    - "Provider detail shows connection event log (rolling 50 events) with retry button and empty state"
  ...
  artifacts:
    ...
    - path: "android/feature/diagnostics/src/test/kotlin/app/dqxn/android/feature/diagnostics/ProviderDetailScreenTest.kt"
      provides: "UI tests for connection event log, empty state, retry button (F7.6)"
    - path: "android/feature/diagnostics/src/test/kotlin/app/dqxn/android/feature/diagnostics/DiagnosticsViewerTest.kt"
      provides: "UI tests for snapshot filtering, session viewer toggle/count/clear, observability metrics display"
  key_links:
...
Output: DiagnosticsViewModel + 5 composable screens + ProviderHealthDashboard UI test + ProviderDetailScreen UI test.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    5. **ProviderDetailScreenTest** (Compose UI test — F7.6):
       - `connection event log renders 50 events` — provide 50 ConnectionEvent objects, verify all 50 rendered via test tags `connection_event_0` through `connection_event_49`
       - `empty state shows no events message` — provide empty events list, verify "No connection events" text displayed
       - `retry button fires callback` — tap retry_button, verify onRetry callback invoked with correct providerId
       - `events sorted newest first` — provide 3 events with ascending timestamps, verify first rendered event has latest timestamp

    Use `createAndroidComposeRule<ComponentActivity>` for UI tests.
  </action>
  <verify>
    <automated>cd /Users/ohm/Workspace/dqxn/android && ./gradlew :feature:diagnostics:testDebugUnitTest --tests "*.ProviderHealthDashboardTest" --tests "*.ProviderDetailScreenTest" --console=plain 2>&1 | tail -20</automated>
  </verify>
  <done>DiagnosticsViewModel wired, ProviderHealthDashboard renders provider list with staleness (5 UI tests), ProviderDetailScreen connection event log verified (4 UI tests) — 9 UI tests pass total</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    4. **DiagnosticsViewerTest** (Compose UI test covering all three viewers):
       - `snapshot filter chips filter by type` — provide mixed snapshot types, tap CRASH filter, verify only CRASH rows visible
       - `snapshot viewer empty state` — provide empty snapshot list, verify "No snapshots" message
       - `recording toggle shows red dot when recording` — set isRecording=true, verify recording indicator semantics on `recording_toggle`
       - `session viewer shows event count` — provide 50 events with max 10000, verify "50 / 10,000" text displayed
       - `clear button fires callback` — tap `clear_button`, verify onClear callback invoked
       - `observability dashboard renders frame metrics` — provide P50=8ms, P95=14ms, verify `frame_time_p50` and `frame_time_p95` test tags render with values

       Use `createAndroidComposeRule<ComponentActivity>`. Mock data via parameters — these are stateless composables.
  </action>
  <verify>
    <automated>cd /Users/ohm/Workspace/dqxn/android && ./gradlew :feature:diagnostics:testDebugUnitTest --tests "*.DiagnosticsViewerTest" --console=plain 2>&1 | tail -20</automated>
  </verify>
  <done>DiagnosticSnapshotViewer, SessionRecorderViewer, ObservabilityDashboard all render correctly — 6 behavioral UI tests pass (filter, empty state, recording toggle, event count, clear, frame metrics)</done>
</task>

</tasks>

<verification>
cd /Users/ohm/Workspace/dqxn/android && ./gradlew :feature:diagnostics:testDebugUnitTest --tests "*.ProviderHealthDashboardTest" --tests "*.ProviderDetailScreenTest" --tests "*.DiagnosticsViewerTest" --console=plain
</verification>
...
- ProviderDetailScreen shows connection event log with retry (4 UI tests pass, F7.6)
- DiagnosticSnapshotViewer filters by type, handles empty state (2 UI tests)
- SessionRecorderViewer shows recording toggle, event count, clear button (3 UI tests)
- ObservabilityDashboard renders frame metrics (1 UI test)
- 15 total tests pass (5 + 4 + 6)
</success_criteria>
