...
must_haves:
  ...
  key_links:
    ...
    - from: "OnboardingViewModel.selectTheme"
      to: "UserPreferencesRepository.setSelectedThemeId"
      via: "persists theme selection so ThemeCoordinator applies it on next launch"
      pattern: "setSelectedThemeId|selectedThemeId"
  ...
  <action>
    1. **OnboardingViewModel** (`@HiltViewModel`):
       ...
       - Methods:
         ...
         - `selectTheme(themeId: String)` — calls `userPreferencesRepository.setSelectedThemeId(themeId)` to persist the selected theme ID. ThemeCoordinator monitors `selectedThemeId` flow and applies the theme on startup. Read the actual UserPreferencesRepository interface to find the correct field name (may be `selectedThemeId`, `activeThemeId`, or similar).
