...
- Direct instantiation of all three handlers (no Hilt)
- Real `ChaosProviderInterceptor()` (no-arg constructor)
- Real `DiagnosticSnapshotCapture` with **MockK-wrapped** `DiagnosticFileWriter` — `DiagnosticFileWriter.checkStoragePressure()` calls Android-only `StatFs` which throws on JVM. Copy the `createMockFileWriter()` helper from `DiagnosticSnapshotCaptureTest`:
  ```kotlin
  ...
  ```
  This stubs `checkStoragePressure()` → `false` while delegating `write()`/`read()` to a real instance backed by `@TempDir`.
...
- `DiagnosticFileWriter` MUST be MockK-wrapped (not raw construction) — `checkStoragePressure()` uses Android `StatFs` which throws `UnsatisfiedLinkError` on JVM. See `createMockFileWriter()` helper above.
