...
must_haves:
  truths:
    ...
    - "F6.3/F6.4 entitlement gating chain: SOLAR_AUTO/ILLUMINANCE_AUTO modes resolve to user-selected light/dark themes, which are gated at selection time via requiredAnyEntitlement -- mode activation itself needs no separate entitlement check"
  artifacts:
    ...
    - path: "android/pack/themes/src/main/kotlin/app/dqxn/android/pack/themes/ThemesThemeModule.kt"
      ...
      contains: "IntoSet"
    - path: "android/pack/themes/src/test/kotlin/app/dqxn/android/pack/themes/ThemeEntitlementGatingChainTest.kt"
      provides: "Proves F6.3/F6.4 gating chain: all premium themes gated, SOLAR_AUTO/ILLUMINANCE_AUTO covered by theme-level entitlement"
...
Purpose: Delivers F6.1 (22 premium themes), F6.2 (themes entitlement gates Theme Studio access), F6.3/F6.4 (SOLAR_AUTO/ILLUMINANCE_AUTO entitlement gating). The gating chain for F6.3/F6.4: premium themes have `requiredAnyEntitlement = setOf("themes")` -> Theme Studio UI gates theme selection -> SOLAR_AUTO/ILLUMINANCE_AUTO modes only resolve between user's already-selected light/dark themes -> modes themselves need no separate entitlement check because they can only activate themes the user has already been entitled to select.
...
<task type="auto">
  ...
  <action>
    ...
    **4. Create ThemeEntitlementGatingChainTest.kt** at `android/pack/themes/src/test/kotlin/app/dqxn/android/pack/themes/ThemeEntitlementGatingChainTest.kt`:
    - Test proving the F6.3/F6.4 gating chain is complete:
      - `all premium themes have requiredAnyEntitlement containing themes` -- load all 22 themes, assert every one has `requiredAnyEntitlement = setOf("themes")`
      - `no premium theme has null requiredAnyEntitlement` -- ensures no theme leaks without gating
      - `gating chain completeness -- SOLAR_AUTO and ILLUMINANCE_AUTO modes resolve via ThemeAutoSwitchEngine which only picks between user-selected light/dark themes -- those themes are gated at selection time by requiredAnyEntitlement` -- this is a documentation-as-test: create a test that loads all themes, verifies all are gated, then documents in test name that SOLAR_AUTO/ILLUMINANCE_AUTO mode activation is covered by theme-level gating (no separate mode-level check needed because the engine only resolves between already-entitled theme selections)
    - This test lives in `:pack:themes` because it validates the theme provider's output

    **5. Create ThemeJsonValidationTest.kt:**
