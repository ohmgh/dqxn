...
<task type="auto">
  ...
  <action>
    ...
    **4. Update DebugModule.kt** — add `@Provides @Singleton` for `ChaosProviderInterceptor` and `ChaosEngine`:
    ...
    Also add `@Binds @IntoSet` for `ChaosProviderInterceptor as DataProviderInterceptor` in DebugModule's abstract section so `WidgetDataBinder` receives it in debug builds. The `@Multibinds fun dataProviderInterceptors(): Set<DataProviderInterceptor>` already exists in `AppModule` (line 40 of `android/app/src/main/kotlin/app/dqxn/android/di/AppModule.kt`) -- do NOT add it again.

    **Note:** ChaosProviderInterceptor and ChaosEngine both have `@Inject constructor()`. Since `:core:agentic` is added via `debugImplementation`, Hilt sees these classes in debug builds and can wire them automatically. The only manual addition needed is the `@Binds @IntoSet` abstract function mapping `ChaosProviderInterceptor` to `DataProviderInterceptor`.
