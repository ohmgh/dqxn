...
CompositionLocalProvider(LocalDashboardTheme provides themeState.displayTheme) {
    Box(Modifier.fillMaxSize()) {
        ProfilePageTransition(profiles, activeProfileId) { profileId ->
            DashboardLayer(layoutState, editState, dragState, resizeState, configBoundaries, ...)
        }
...
**Reduced motion tests (NF39):** These automate what VALIDATION.md previously marked as manual-only. Inject `ReducedMotionHelper` with `isReducedMotion = true` via the harness, then verify downstream behavior:
- `reduced motion: edit mode wiggle animation disabled` — enter edit mode with reduced motion enabled, verify `EditModeCoordinator.editState` has `animationsEnabled = false` (or equivalent flag consumed by DashboardGrid wiggle animation)
- `reduced motion: widget add/remove transitions are instant` — verify DashboardGrid uses `snap()` spec instead of spring when ReducedMotionHelper reports reduced motion
- `reduced motion: profile page transition is instant` — verify ProfilePageTransition disables HorizontalPager scroll animation when reduced motion active
...
- Reduced motion tests pass (NF39 — previously manual-only)
