...
must_haves:
  ...
  key_links:
    ...
    - from: "LayoutCoordinator"
      to: "ConfigurationBoundaryDetector"
      via: "constructor injection — exposes boundaries StateFlow for grid + edit mode"
      pattern: "configurationBoundaryDetector.*ConfigurationBoundaryDetector"
    - from: "ConfigurationBoundaryDetector"
      to: "WindowInfoTracker"
      via: "constructor injection — observes foldable display features"
      pattern: "windowInfoTracker.*WindowInfoTracker"
...
<task type="auto">
  ...
  <action>
**build.gradle.kts:** Add dependencies beyond what `dqxn.android.feature` auto-provides. Add: `implementation(project(":core:design"))`, `implementation(project(":core:thermal"))`, `implementation(project(":data"))`, `implementation(project(":sdk:analytics"))`, `implementation(libs.window)` (for `WindowInfoTracker` — foldable configuration boundary detection). Enable testFixtures: `android { testFixtures { enable = true } }`. Add `testImplementation(testFixtures(project(":sdk:contracts")))` for `TestDataProvider`. Add Turbine, Truth, MockK test deps. Add `testFixturesImplementation` for kotlinx-coroutines-test, Truth.
...
**LayoutCoordinator.kt:** Package `app.dqxn.android.feature.dashboard.coordinator`. `@Inject constructor` with `layoutRepository: LayoutRepository`, `presetLoader: PresetLoader`, `gridPlacementEngine: GridPlacementEngine`, `configurationBoundaryDetector: ConfigurationBoundaryDetector`, `@param:IoDispatcher ioDispatcher: CoroutineDispatcher`, `logger: DqxnLogger`.
...
`val configurationBoundaries: StateFlow<ImmutableList<ConfigurationBoundary>>` — exposes boundaries from `configurationBoundaryDetector.boundaries`. Collected by DashboardScreen and passed to DashboardGrid + DashboardLayer for no-straddle enforcement and edit-mode boundary lines.

Methods: `initialize(scope: CoroutineScope)` — loads from `layoutRepository.observeWidgets()` on IO, updates `_layoutState`. Also starts `configurationBoundaryDetector.observe(scope)` to begin boundary detection. `handleAddWidget(widget)` — uses `gridPlacementEngine.findOptimalPosition()` passing current `configurationBoundaries.value`, updates state, persists. `handleRemoveWidget(widgetId)` — removes from state, cancels via WidgetBindingCoordinator (callback interface), persists. `handleMoveWidget(widgetId, position)` — updates position, persists. `handleResizeWidget(widgetId, size, position)` — updates size + optional position (non-BottomRight handles), persists. `handleResetLayout()` — reloads from PresetLoader. `visibleWidgets(viewportCols, viewportRows): ImmutableList<DashboardWidgetInstance>` — viewport culling filter.

Layout save via `layoutRepository` which already debounces at 500ms (Phase 5). No additional debouncing needed in coordinator.

**ConfigurationBoundaryDetector.kt:** Package `app.dqxn.android.feature.dashboard.grid`. `@Inject constructor` with `windowInfoTracker: WindowInfoTracker`, `logger: DqxnLogger`.

`val boundaries: StateFlow<ImmutableList<ConfigurationBoundary>>` — emits current configuration boundaries.

`fun observe(activity: Activity, scope: CoroutineScope)` — Activity reference passed at observation time (from DashboardScreen/ViewModel init via `LocalContext.current as Activity`). Collects `windowInfoTracker.windowLayoutInfo(activity)`. For foldable devices: extracts `FoldingFeature` from `WindowLayoutInfo.displayFeatures`, converts fold bounds to `ConfigurationBoundary(name = "fold", rect = foldFeature.bounds.toIntRect())`. For non-foldable devices: computes the alternate-orientation viewport rectangle using `activity.resources.configuration` and `activity.resources.displayMetrics` — landscape width/height swapped to produce the portrait boundary and vice versa. Result: `ConfigurationBoundary(name = "portrait"/"landscape", rect = IntRect(0, 0, altWidth, altHeight))`. Emits `emptyList()` if device has a single fixed orientation (NF45 config-aware presets account for this).
...
**ConfigurationBoundaryDetectorTest.kt:** JUnit5 + `@Tag("fast")` + MockK. Mock `WindowInfoTracker`. Tests:
- `foldable device emits fold boundary from FoldingFeature` — mock `windowLayoutInfo()` with a `FoldingFeature`, verify `ConfigurationBoundary` with fold rect emitted
- `non-foldable device emits alternate orientation boundary` — mock empty `displayFeatures`, verify alternate-orientation boundary computed from display metrics
- `fixed orientation device emits empty boundaries` — single fixed orientation, verify `emptyList()`
- `boundary updates on configuration change` — emit new windowLayoutInfo, verify boundaries flow updates
