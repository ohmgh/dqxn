...
**WidgetSlot.kt:** Package `app.dqxn.android.feature.dashboard.binding`. `@Composable fun WidgetSlot(widget: DashboardWidgetInstance, widgetBindingCoordinator: WidgetBindingCoordinator, widgetRegistry: WidgetRegistry, modifier: Modifier = Modifier)`.
...
6. Apply `Modifier.semantics { contentDescription = renderer.accessibilityDescription(data) }` for F2.19 widget accessibility.
7. Apply `Modifier.testTag("widget_${widget.instanceId}")`.
...
**UnknownWidgetPlaceholder.kt:** In `:sdk:ui`. `@Composable fun UnknownWidgetPlaceholder(typeId: String, modifier: Modifier = Modifier)`. Renders "Unknown widget: {typeId}" with icon. Used when `WidgetRegistry.findByTypeId()` returns null (F2.13).

**LocalWidgetPreviewUnits.kt:** In `:sdk:ui`. `val LocalWidgetPreviewUnits = compositionLocalOf<GridSize?> { null }`. CompositionLocal providing target dimensions during resize gesture. 8 widgets read this for content-aware relayout (Speedometer arc angle, InfoCardLayout mode switching).
  </action>
  <verify>
  ...
  <done>DashboardGrid renders widgets via custom Layout with viewport culling and graphicsLayer isolation. WidgetSlot provides error boundary with LocalWidgetData. DashboardLayer assembles Layer 0 with semantics registration. DashboardButtonBar auto-hides with 76dp touch targets. Banner hosts handle notification display.</done>
</task>
...
<verification>
- `./gradlew :feature:dashboard:compileDebugKotlin :sdk:ui:compileDebugKotlin --console=plain` succeeds
