...
<task type="auto">
  ...
  <action>
**DashboardGrid.kt:** Package `app.dqxn.android.feature.dashboard.grid`. `@Composable fun DashboardGrid(widgets: ImmutableList<DashboardWidgetInstance>, viewportCols: Int, viewportRows: Int, editState: EditState, dragState: DragUpdate?, resizeState: ResizeUpdate?, configurationBoundaries: ImmutableList<ConfigurationBoundary>, widgetBindingCoordinator: WidgetBindingCoordinator, widgetRegistry: WidgetRegistry, reducedMotionHelper: ReducedMotionHelper, onCommand: (DashboardCommand) -> Unit, modifier: Modifier = Modifier)`.
...
Configuration boundary lines (F1.26): when `editState.isEditMode`, overlay boundary line composables with labels ("folded portrait", etc.). Lines from `configurationBoundaries: ImmutableList<ConfigurationBoundary>` — received as a parameter from DashboardScreen, which collects it from `layoutCoordinator.configurationBoundaries`. Passed through to `DashboardGrid` for no-straddle snap enforcement during drag/resize.
...
**Foldable no-straddle snap tests (NF46):** These automate what VALIDATION.md previously marked as manual-only. Mock `ConfigurationBoundaryDetector` to emit fold boundaries, then verify grid placement logic:
- `widget straddling fold boundary snapped to nearest side` — place widget at fold line, verify GridPlacementEngine relocates it fully to one side
- `widget fully within one fold region is not relocated` — widget not touching boundary stays put
- `fold boundary change triggers re-validation of all widget positions` — emit new fold state, verify all widgets re-checked against new boundary
- `no-straddle enforced during drag: snap preview respects fold boundary` — drag a widget toward fold line, verify snap candidate is on one side
...
- `isInteractionAllowed false renders widget in non-interactive mode` (F2.18) — mock editModeCoordinator returning false, verify renderer receives non-interactive flag
  ...
  <done>DashboardGrid tests verify viewport culling, z-index ordering, and foldable no-straddle snap (NF46). WidgetSlot tests verify error boundary, unknown widget placeholder, status overlays, and accessibility.</done>
</task>
...
- Foldable no-straddle snap tests pass (NF46 — previously manual-only)
