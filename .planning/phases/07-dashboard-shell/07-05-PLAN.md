...
- `@Provides @Singleton fun provideWindowInfoTracker(@param:ApplicationContext context: Context): WindowInfoTracker = WindowInfoTracker.getOrCreate(context)` — required by ConfigurationBoundaryDetector (Plan 01). `WindowInfoTracker.getOrCreate()` accepts `Context` (not just Activity). The detector calls `windowInfoTracker.windowLayoutInfo(activity)` at observation time when it receives the Activity reference.
