...
- `toggleStatusBar()` — toggles `showStatusBar` in EditState. Called by DashboardViewModel on ToggleStatusBar command. DashboardLayer (Plan 06) reads `editState.showStatusBar` to show/hide system bars via `WindowInsetsControllerCompat`.
...
- `toggleStatusBar toggles showStatusBar in EditState`
