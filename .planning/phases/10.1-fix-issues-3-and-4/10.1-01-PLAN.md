...
autonomous: true
requirements: []  # NF27 in ROADMAP is a tagging error — NF27 (minSdk 31) was fully satisfied in Phase 1. This phase addresses KSP codegen correctness, not SDK version config.
...
must_haves:
  truths:
    ...
    - "Generated manifest keeps themes = persistentListOf() at codegen time (individual theme IDs are runtime data from ThemeProvider.getThemes()) -- runtime theme catalog served by Set<ThemeProvider> Hilt multibinding, not manifest"
...
<task type="auto">
  <name>Task 1: Fix ManifestGenerator + HiltModuleGenerator + add ThemeProvider codegen + update test infrastructure</name>
  <files>
    android/sdk/contracts/src/main/kotlin/app/dqxn/android/sdk/contracts/annotation/DashboardThemeProvider.kt
    android/codegen/plugin/src/main/kotlin/app/dqxn/android/codegen/plugin/model/ThemeProviderInfo.kt
    android/codegen/plugin/src/main/kotlin/app/dqxn/android/codegen/plugin/handlers/ThemeProviderHandler.kt
  ...
  <action>
    ...
    **8. Update test infrastructure** (required so existing tests pass after production code changes):

    a. **Fix `ContractStubs.kt` PackCategory enum** in `manifestStubs()`:
    Change `PLUS, THEMES, DEMO` to `PREMIUM, REGIONAL, DEBUG` matching the real enum.

    b. **Add ThemeProvider + DashboardThemeProvider stubs** to `contractStubs()`:
    - `ThemeProvider.kt` stub: `interface ThemeProvider { val packId: String }`
    - `DashboardThemeProvider.kt` stub: annotation with `@Retention(SOURCE) @Target(CLASS)`

    c. **Add Dagger `@Provides` stub** to `daggerStubs()`:
    ```kotlin
    ...
    d. **Add `PackThemeRef` stub** and update `DashboardPackManifest` stub `themes` field type from `List<Any>` to `List<PackThemeRef>`.

    e. **Update `compile()` helper** in `PluginProcessorTest`: add `packCategory: String = "ESSENTIALS"` parameter, pass as `processorOptions["packCategory"] = packCategory`.

    f. **Update existing test assertions** broken by always-generated HiltModule:
    - `module with no annotations produces only manifest`: HiltModule IS now generated (contains manifest @Provides). Change assertion from `doesNotContain("EssentialsHiltModule.kt")` to `contains("EssentialsHiltModule.kt")` and verify it has `provideManifest` but no `@Binds` methods.
    - `valid DashboardWidget generates Hilt module`: Also assert `provideManifest` exists in generated HiltModule.
    - `PackManifest object is generated with widget and provider refs`: Assert `PackCategory.ESSENTIALS` appears explicitly.
  </action>
  <verify>
    <automated>cd android && ./gradlew :codegen:plugin:test --console=plain 2>&1 | tail -20</automated>
    Runs all existing compile-testing tests against the updated production code + fixed stubs. Proves codegen changes don't regress existing behavior.
  </verify>
  <done>
    ...
    - Test stubs fixed (PackCategory enum, ThemeProvider, @Provides)
    - Existing compile-testing tests updated and passing
  </done>
</task>

<task type="auto">
  <name>Task 2: Add new codegen tests + annotate pack providers + delete manual theme modules</name>
  <files>
    android/codegen/plugin/src/test/kotlin/app/dqxn/android/codegen/plugin/PluginProcessorTest.kt
    android/pack/essentials/src/main/kotlin/app/dqxn/android/pack/essentials/theme/EssentialsThemeProvider.kt
    android/pack/essentials/src/main/kotlin/app/dqxn/android/pack/essentials/theme/EssentialsThemeModule.kt
  ...
  <action>
    **1. Add new tests** to `PluginProcessorTest.kt` (test infrastructure already updated in Task 1):
    ...
    **2. Add `@DashboardThemeProvider` to pack providers:**
  ...
  <verify>
    <automated>cd android && ./gradlew :codegen:plugin:test :pack:essentials:compileDebugKotlin :pack:themes:compileDebugKotlin --console=plain 2>&1 | tail -20</automated>
    Runs all codegen tests (existing + 5 new) AND verifies both packs compile with @DashboardThemeProvider and without manual modules.
  </verify>
  <done>
    ...
    - All codegen tests pass (existing + new)
    - Both packs compile without manual theme modules
  </done>
</task>
