...
- F4.6  -> 14-05 + 14-06 (preview overlay + preview timeout)
...
class Phase14RegressionTest {
    ...
    @Test
    fun `F4_6 theme preview timeout constant exists`() {
        val coordinatorFile = File(
            projectDir,
            "feature/dashboard/src/main/kotlin/app/dqxn/android/feature/dashboard/coordinator/ThemeCoordinator.kt"
        )
        val content = coordinatorFile.readText()
        assertThat(content).contains("PREVIEW_TIMEOUT_MS")
        assertThat(content).contains("60_000")
    }
