...
must_haves:
  truths:
    ...
    - ":feature:onboarding test has an allowlist for FirstRunFlow.kt, AnalyticsConsentStep.kt, FirstLaunchDisclaimer.kt (deferred to Phase 15), and ProgressiveTip.kt"
...
<!-- Current MaterialTheme usage by module (as of pre-Phase-14):
     ...
     :feature:onboarding (4 files use MaterialTheme, all deferred to Phase 15):
 ...
 * instead of MaterialTheme.
 *
 * Onboarding files (FirstRunFlow, AnalyticsConsentStep, FirstLaunchDisclaimer) deferred
 * to Phase 15 (setup schema framework integration). All 4 onboarding files allowlisted.
 */
@RunWith(RobolectricTestRunner::class)
class DesignTokenWiringTest {
...
- These tests will initially FAIL if run before plans 14-06 and 14-09 execute (since those plans haven't migrated the files yet). This plan is Wave 1 alongside those plans. The tests are designed to pass in the post-migration state.
