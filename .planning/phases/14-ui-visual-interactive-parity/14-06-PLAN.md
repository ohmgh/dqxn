...
must_haves:
  truths:
    - "ThemeSelector uses GridCells.Fixed(3) -- 3 columns, 4 rows visible"
  ...
  artifacts:
    - path: "android/feature/settings/src/main/kotlin/app/dqxn/android/feature/settings/theme/ThemeSelector.kt"
      provides: "3-col 2-page HorizontalPager theme selector with gradient backgrounds and color dots"
    ...
    - path: "android/feature/settings/src/test/kotlin/app/dqxn/android/feature/settings/theme/ThemeSelectorTest.kt"
      provides: "Tests for 3-col grid, pager, color dots, star icon, no timeout, isDark filtering"
      contains: "GridCells.Fixed(3)"
---

<objective>
Rework ThemeSelector to 3-column 2-page HorizontalPager layout with visual parity from old codebase.

Purpose: Current ThemeSelector has a 3-column single grid (correct column count), but no HorizontalPager, no gradient backgrounds on theme cards, no color-dot swatches, lock icon instead of star for premium, wrong aspect ratio (1.5f vs 2f), wrong selection border color (accentColor vs highlightColor), and a 60s preview timeout that the old codebase didn't have. The grid should show 4 rows visible. This plan fixes ALL ThemeSelector visual issues and adds isDark/onCreateNewTheme parameters that 14-09 wires.
...
<!-- Old codebase ThemeSelector layout (user corrections applied):
     ...
     - LazyVerticalGrid with GridCells.Fixed(3) -- 3 columns, 4 rows visible
...
<!-- Current ThemeSelector state (deltas to fix):
     1. Keep GridCells.Fixed(3) -- 3 columns is correct (matches old codebase)
...
<task type="auto">
  <name>Task 1: Rework ThemeSelector to 3-col HorizontalPager with visual parity</name>
  <files>
    android/feature/settings/src/main/kotlin/app/dqxn/android/feature/settings/theme/ThemeSelector.kt
  </files>
  <action>
...
private fun ThemeGrid(
    ...
    LazyVerticalGrid(
        columns = GridCells.Fixed(3),  // 3 columns, 4 rows visible (matches old codebase)
...
The `sortThemes()` function at the bottom remains unchanged.
  ...
  <done>
    - ThemeSelector uses GridCells.Fixed(3) with 4 rows visible
...
Check the existing createTheme helper. It currently uses `DashboardThemeDefinition(...)` but doesn't pass `highlightColor`. If `highlightColor` has a default value in DashboardThemeDefinition, that's fine. If not, add `highlightColor = Color.Yellow` to the helper.
  ...
  <done>
    ...
    - Source verification: GridCells.Fixed(3), HorizontalPager, no PREVIEW_TIMEOUT_MS
...
<success_criteria>
- ThemeSelector: 3-col grid (4 rows visible), 2-page HorizontalPager, gradient backgrounds, 4 color dots, premium stars (not locks), highlightColor selection border, 2f aspect ratio
