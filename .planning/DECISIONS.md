...
| Why `ProviderFault` in `:sdk:contracts` main source set? | Main source set because `ChaosProviderInterceptor` needs it at debug runtime, not just test time. `DashboardTestHarness` also consumes it via testFixtures → main dependency. |
...
| Why defer the Plus Pack (F5B.1–F5B.5) to post-launch? | V1 scope control. Essentials pack validates the full architecture. Plus pack adds widget variety, not architectural surface area — low risk to defer. |
